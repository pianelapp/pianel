    ...
    <div
      ...
      <div data-hf-device className="flex items-center gap-2 mb-2">
        <span className={`min-w-0 truncate text-sm font-medium ${bodyClass}`}>
          {surface.device ? (surface.device.name ?? surface.device.id) : 'No footswitch'}
        </span>
        {surface.device && (
          <span
      ...
      <div data-hf-monitor className={`font-mono text-xs mb-3 truncate ${bodyClass}`}>
      ...
      {groups.map(group => (
        <div key={group.name} data-hf-group={group.name} className="mb-2">
          <span
            className={`block font-mono text-[10px] font-bold tracking-widest mb-1 ${bodyClass}`}>
        ...
        <div data-hf-orphan className="mt-1">
          {surface.orphanBindings.map(binding => (
            <div key={binding.id} className="flex items-center gap-2">
              <span className={`min-w-0 truncate font-mono text-xs ${waitingClass}`}>
