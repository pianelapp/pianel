...
export function StatusBar({ isLightMode, compact = false }: StatusBarProps) {
  const {
    volume,
    tempo,
    metronomeOn,
      ...
      <div
        ...
        <div className={`flex items-center ${compact ? 'gap-3' : 'gap-6'}`}>
          ...
          <button
            ...
            className={`${compact ? 'tap-target ' : ''}font-mono text-xl cursor-pointer hover:opacity-80 transition-opacity ${
              isLightMode ? 'text-zinc-700' : 'text-zinc-300'
            }`}
        ...
        <div className="relative">
          <button
            ...
            aria-label={compact ? 'Volume' : undefined}
            className={`${compact ? 'tap-target ' : ''}flex items-center gap-2.5 pl-3 py-1.5 rounded-xl transition-colors ${
              isLightMode ? 'hover:bg-zinc-200' : 'hover:bg-zinc-800'
            }`}>
            {headphonesConnected === true ? (
              <Headphones
                className="w-5 h-5 text-zinc-500"
                aria-label="Headphones connected"
              />
            ) : (
              <Volume2
                className="w-5 h-5 text-zinc-500"
            ...
            {!compact && (
              <span
                className={`font-mono text-xl inline-block min-w-[3ch] text-left tabular-nums ${isLightMode ? 'text-zinc-700' : 'text-zinc-300'}`}>
                {volume}
              </span>
            )}
          </button>
          ...
          {showVolume && (
            <div
              ...
              <div
                ...
                <div
                  className={`text-4xl font-light tracking-tighter ${isLightMode ? 'text-zinc-800' : 'text-zinc-100'}`}>
                  {volume}%
                </div>
                <div
                  className={`text-xs font-bold tracking-widest mt-1 mb-5 ${isLightMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
                  VOL
                </div>
                <div
                  className={`text-xs font-bold mb-2 ${isLightMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
                  100%
                </div>
                <div
                  ...
                  <div
                    ...
                    style={{ height: `${volume}%` }}>
                    <div
                      className={`absolute top-0 left-0 right-0 h-[3px] ${isLightMode ? 'bg-zinc-800' : 'bg-cyan-200/80'}`}
                    />
                  </div>
                </div>
                <div
                  className={`text-xs font-bold mt-2 mb-6 ${isLightMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
                  0%
                </div>
                <button
                  onClick={() => setShowVolume(false)}
